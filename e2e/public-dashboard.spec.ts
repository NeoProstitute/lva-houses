...
test("public dashboard supports English and Czech without horizontal overflow", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Body týmů na první pohled." })).toBeVisible();
  ...
  await expect(page.getByText("Svazek klíčů", { exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Vývoj po jednotlivých týdnech." })).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBeTruthy();
